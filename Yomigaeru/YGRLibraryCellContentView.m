...
@interface YGRLibraryCellContentView ()
...
@implementation YGRLibraryCellContentView
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    [self addSubview:_unreadBubbleView];
...
- (void)layoutSubviews
{
    ...
    if (!self.unreadBubbleView.hidden) {
        ...
        CGFloat bubblePadding = 6.0f;

        CGSize textSize = [self.unreadLabel sizeThatFits:CGSizeMake(CGFLOAT_MAX, minSize)];
        CGFloat width = MAX(minSize, textSize.width + bubblePadding);
